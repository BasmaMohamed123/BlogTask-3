  ...
  <div v-if="post" class="post-details">
    ...
    <div class="comments-section" v-if="comments.length">
      ...
      <div v-for="comment in sortedComments" :key="comment.id" class="comment">
        ...
        <div v-if="isCommentOwner(comment) || isPostOwner">
          <button @click="editComment(comment)" class="button edit-button">Edit</button>
    ...
    <!-- Editing Comment -->
    <div v-if="editingComment" class="editing-comment">
...
export default defineComponent({
  ...
  methods: {
    ...
    async deleteComment(commentId: number) {
      ...
      if (!this.isCommentOwner(this.comments.find(comment => comment.id === commentId)!)
        && !this.isPostOwner) {
    ...
    async updateComment() {
      ...
      try {
        ...
        if (commentIndex !== -1) {
          this.comments.splice(commentIndex, 1, updatedComment); // Replace the old comment with the updated one
        }
        this.cancelEdit();
      } catch (error) {
